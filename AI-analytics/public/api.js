...
window.Api = (() => {
  ...
  // ---------------- AISUM (normalize) ----------------
  // 1) umumiy (APP + TG) — eski chartlar ishlatadi
  const fetchAisumAll = async () => {
    ...
    const tg  = await selectAll(
      'aisum_tgbot',
      'created_at, ai_message, topic, doctor, chat_id, "group"',
      q => q.order('created_at', { ascending: true })
    );
    const appN = app.map(r => ({ ...r, doctor: null, _src: 'app' }));
    const tgN  = tg .map(r => ({ ...r, gender: null, _src: 'tg' }));
    return [...appN, ...tgN];
  };

  // 2) faqat TG (pie shu bilan ishlaydi)
  const fetchAisumTgOnly = async () => {
    const tg  = await selectAll(
      'aisum_tgbot',
      'created_at, ai_message, topic, doctor, chat_id, "group"',
      q => q.order('created_at', { ascending: true })
    );
    // bu yerda APP yo‘q → faqat tg
    return tg.map(r => ({ ...r, gender: null, _src: 'tg' }));
  };
